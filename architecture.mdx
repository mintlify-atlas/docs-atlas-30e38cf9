---
title: "Two-level LangGraph architecture of the ERP Agent"
sidebarTitle: "Architecture"
description: "How the outer Conversation Graph and inner SQL Pipeline Graph interact, including routing logic, the self-correction loop, HITL gate, and Schema Registry design."
---
...
When `sql_validator` finds errors it routes to `error_handler`, which rewrites the generation prompt with the specific error context and feeds it back into `sql_validator`. The loop runs a maximum of **3 attempts** (`MAX_CORRECTION_ATTEMPTS`). After that, or if a security violation is detected, the graph routes to `give_up → response_formatter`.
...
| **Schema index** | FAISS CPU (`faiss-cpu >= 1.8.0`) |
| **Infrastructure** | AWS ECS Fargate + ALB + NAT Gateway |
| **Observability** | Prometheus (`prometheus-client`) + structlog + LangSmith tracing |
| **Forecasting** | StatsForecast (`statsforecast >= 1.7.0`) |
| **SQL parsing** | sqlglot (`sqlglot >= 26.0.0`) |
