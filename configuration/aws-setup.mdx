...
|---|---|---|
| `platform_admin` | Platform administrator | Full access including RBAC overrides and cross-tenant operations |
| `org_admin` | Organisation administrator | User management, all financial data modules |
| `finance_lead` | Finance lead | All financial modules, export, file upload |
...
2. Fetches the JWKS document from `https://cognito-idp.{region}.amazonaws.com/{user_pool_id}/.well-known/jwks.json` (cached for 1 hour, auto-refreshed on key rotation).
