...
description: "How the schema registry uses FAISS vector search, YAML catalogs, and a MetricsRegistry to map natural language questions to the right ERP tables and columns."
---
...
In addition to the programmatic `DomainCatalog`, the registry supports **60+ ERP tables** documented in YAML files under the `yaml_schemas/` directory. When `use_generated_schema_catalog=True` (the preferred setting), `CatalogFromYaml` builds the full domain catalog from these YAML files at startup, replacing the hardcoded Python catalog.

This allows schema updates (new tables, column renames, new join hints) to be made in YAML without modifying Python code.
