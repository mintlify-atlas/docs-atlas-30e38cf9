...
sidebarTitle: "Memory"
description: "How the ERP agent combines Redis short-term session memory with DynamoDB long-term conversation history to give Claude cross-turn and cross-session context."
---
...
| `schema:metadata` | 3600 s | Serialized `INFORMATION_SCHEMA` table/column cache |
...
At the start of each new conversation, the agent fetches up to 10 recent session summaries for the user and builds a `session_digest` string:
...
Per-session LLM cost tracking (token counts, estimated USD cost) is persisted with a **Redis-first, DynamoDB fallback** strategy:

1. After each node execution, cost data is written to `session:{session_id}` metadata in Redis.
2. When the session ends, the aggregated cost is included in the DynamoDB session summary.
3. If Redis is unavailable, the cost data falls back to DynamoDB directly.

This ensures cost visibility survives Redis eviction under memory pressure.
