...
description: "How the ERP agent isolates each customer in its own ECS service with a dedicated MySQL schema, and how cross-instance access is managed via Cognito and DynamoDB."
---
...
1. The tenant instance's `expected_tenant` is a specific slug, so `platform_admin` users are required.
